...
@Component
public class WhatsAppRecoveryExecutor implements RecoveryActionExecutor {

    private static final Logger log = LoggerFactory.getLogger(WhatsAppRecoveryExecutor.class);

    private final WhatsAppProvider whatsappProvider;
    private final RecoveryLinkService recoveryLinkService;

    public WhatsAppRecoveryExecutor(WhatsAppProvider whatsappProvider,
    ...
    public boolean supports(RecoveryChannel channel) {
        return channel == RecoveryChannel.WHATSAPP;
    }

    @Override
    public ExecutionResult execute(RecoveryAttempt attempt, RecoveryCase recoveryCase) {
        log.info("Executing WhatsApp recovery action for attemptId={}, caseId={}",
        ...
        CommunicationDeliveryResult deliveryResult = whatsappProvider.sendWhatsApp(request);
